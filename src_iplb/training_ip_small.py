...
log_file_path = "/code/training_ip_small.log"
...
#     print(sample["dual"])
#     print(torch.max(sample["dual"]))
#     print(torch.min(sample["dual"]))
    del sample
...
parser.add_argument("--epoch", help="num of epoch", default="10")
parser.add_argument("-i","--ident", type=str, default="ip")
...
#ip1e5_1000_new_1000
n_Samples = 'ip1e5_1000_new_1000_extra'
n_Samples = args.ident
logging.info(f"pkl path is {n_Samples}")
...
if not os.path.exists('../models/'):
    os.mkdir('../models/')
model_path = '../models/ip_small_' + n_Samples + '_s' + str(embSize) + '.pkl'
set_seed(0)
...
while epoch <= max_epochs:
    ...
    for v_indx in range(n_train):
        ...
        savepath = f'../data/ip_small/ip_small/lp_{int(sample_idx)}.pkl'
        fgz= gzip.open(savepath,'rb') 
