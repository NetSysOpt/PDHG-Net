...
log_file_path=f'../records/{args.code}/ip_{args.stepsize}_{args.primalweight}.log'
...
model_path = '../data/models/'+ 'ip_small_' + str(n_Samples) + '_s' + str(embSize) + '.pkl'
# set_seed(0)
...
for i in range(n_files[1]):
    ...
    fsol=gzip.open(f'../data/sol_pkl/ip_gen/ip_{args.stt+i}.pkl','wb')
    pickle.dump([sol_map,dual_map],fsol)
    fsol.close()
    st=modelz.export_to_mps_string()
    fsol=gzip.open(f'../data/sol_pkl/ip_gen/ip_{args.stt+i}.mps.gz','wb')
