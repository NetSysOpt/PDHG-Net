...
#     solver.SetPrimalTolerance(1e-4)
...
#         print(tmp)
    ...
    # solving:
    if solve:
        solver.set_solver_specific_parameters('termination_criteria {simple_optimality_criteria {eps_optimal_relative: 1e-2}}')
...
def set_from_model2(new_sol_map,new_dual_map,model):
    ptc=model.export_to_proto()
    mdl = get_prob(ptc)
    params = solvers_pb2.PrimalDualHybridGradientParams()
    params.termination_criteria.iteration_limit=0
    params.initial_step_size_scaling=stp
    params.initial_primal_weight=pwg
    opt_criteria = params.termination_criteria.simple_optimality_criteria
    opt_criteria.eps_optimal_relative = 1.0e-4
    ...
    print('    Primal residual:',result.solve_log.solution_stats.convergence_information[0].l2_primal_residual)
    print('    Dual residual:',result.solve_log.solution_stats.convergence_information[0].l2_dual_residual)
...
stp=args.stepsize
pwg=args.primalweight

n_files = [10, args.ntest]
dataset = []    
fp=gzip.open('../data/benchmarks/lb/lp_1.pkl','rb')
pk=pickle.load(fp)
fp.close()
print('Pickle Loaded')
...
n_Samples = 'lb1e5_1000_new_1000_epoch_1'
...
model_path = '../models/'+ 'lb_' + str(n_Samples) + '_s' + str(embSize) + '.pkl'
# set_seed(0)
...
for i in range(n_files[1]):
    modelz = BuildRandomizedModels(i+10000+args.stt)
    data = getFeat(modelz.clone(), False)
    solution, dual = process(model, data, device)
    val_list = []
