...
import psutil
import pickle
log_file_path = "../records/project_log_new_123.log"
...
def process(model, dataset, device):
    ...
    dataset["con_feat"] = dataset["con_feat"].cpu()
    dataset["edge_index"] = dataset["edge_index"].cpu()
    dataset["edge_weight"] = dataset["edge_weight"].cpu()
    dataset["var_feat"] = dataset["var_feat"].cpu()
    dataset["label"] = dataset["label"].cpu()
    dataset["b"] = dataset["b"].cpu()
    dataset["c"] = dataset["c"].cpu()
    dataset["dual"] = dataset["dual"].cpu()
...
dataset = []    
for i in range(1):
    model1 = generate_pagerank_mps_dataset(num_nodes, degree, damping_factor, f'pkl/valid/pkl_{i}.pkl')
#     time31, sol2 = assignHint(model1.clone(), None, None)
    data, solver = getFeat(model1.clone(), True)
#     quit()
    dataset.append([data, model1])

n_Samples = '10e6pagerank'
# n_Samples = 10000
# print(dataset)
...
model_path = '../models/'+ '_d' + n_Samples + '_s' + str(embSize) + '.pkl'
# set_seed(0)
...
for i in range(n_files[1]):
    modelz = generate_pagerank_mps_dataset(num_nodes, degree, damping_factor, f'pkl/valid/pkl_{i}.pkl', seed=i)
    data, solver = getFeat(model1, True)
    time11 = time.time()
    solution, dual, err1, err2,err3, err4, time11,names = process(model, data, device)
    time12 = time.time()
    logging.info(f"第{i}个数据集二范数误差是{err1},第{i}个数据集无穷范数误差是{err2}")
    logging.info(f"第{i}个数据集dual var的二范数误差是{err3},第{i}个数据集无穷范数误差是{err4}")
    ...
    time3,iter3 = set_model(modelz)
    time2,iter2 = set_from_model2(x,y,modelz)
...
## Save csv
name = "time_vs_distance"
df1.to_csv("../records/"+name+"1e6.csv",index=False)
